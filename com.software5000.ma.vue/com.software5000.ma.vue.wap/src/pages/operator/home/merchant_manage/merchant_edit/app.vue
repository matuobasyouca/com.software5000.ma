<template>
    <div class="merchant-insert-page" v-show="!isLoading">
        <ma-head :home="false">编辑</ma-head>
...
<script>
    ...
    import {isMobile, isTel, turnToNextPage, getDataFromParam} from '../../../../../assets/js/utils';
    export default {
        components: {
            maHead,
            cityPicker
                        ...
                        (data, modal)=> {
                            ...
                            if (data.code === 'ZS011000') {
                                ...
                                this.finalData = {
                                    ...
                                    businessTel: data.data.businessTel,
                                    businessAreaCode: data.data.businessAreaCode,
                                    businessAddress: data.data.businessAddress,
                                    businessDetail: data.data.businessDetail,
                                    imgPath: data.data.imgPath,
                                    businessCoordinate: data.data.businessCoordinate,
                                    businessLineState: data.data.businessLineState,
                                }
                                this.imgShow = data.data.imageShowPath;
                                this.tempAddr = data.data.areaList;
                                this.merchantState = data.data.businessLineState == 1;
                                this.setAddressShow(this.tempAddr);
        ...
        mounted(){
            this.finalData.businessId = getDataFromParam('businessId');
            //获取商家详情
            this.handldGetMerchantInfo(this.finalData.businessId)
            //创建百度地图对象
...
<style lang="less">
    @import '../merchant_insert/insert.less';
</style>